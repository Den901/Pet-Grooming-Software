...
Release corrente: `1.0.20` stabile.
...
- drag and drop appuntamenti su desktop/tablet attivo di default e disattivabile da Impostazioni: in settimana spostamento diretto sul giorno, in mese apertura del planning giornaliero dopo 2 secondi, uscita dal popup giorno per passare a un'altra giornata, appuntamento flottante centrato sotto il mouse durante il trascinamento, nel planning scelta precisa dell'orario a scatti di 5 minuti e chiusura automatica dopo il rilascio; su telefono e disattivato; movimento alleggerito con anteprima mirata;
...
- ricerca nel menu Schede ottimizzata per Android: il campo non viene piu ricreato durante la digitazione;
- ricerca schede;
...
- `Pet-Grooming-Software-1.0.20-windows.zip`;
- `Pet-Grooming-Software-1.0.20-linux.tar.gz`;
- `Pet-Grooming-Software-1.0.20.pgs-update`;
...
1. Estrai `Pet-Grooming-Software-1.0.20-windows.zip`.
...
1. Copia `Pet-Grooming-Software-1.0.20-linux.tar.gz` sul server.
2. Estrai il pacchetto e entra nella cartella:

```bash
tar -xzf Pet-Grooming-Software-1.0.20-linux.tar.gz
cd Pet-Grooming-Software-1.0.20
```
...
La release corrente e `1.0.20` stabile. Per preparare i pacchetti della versione impostata in `package.json`:
